...
1. 댓글 등록 기능<span style="color:red;">(엉망이라 강사님꺼보고 베껴야함)</span>

   - 화면구성(detail.jsp)

     - 댓글 입력할 수 있는 textarea와 등록버튼을 추가

     - button과 textarea에  클래스 추가

     - form태그로 감싸지않음(ajax로 할거니까)

       - ajax는 Controller에서 하는거랑 뭔가다름 ㅜ 하 어려워
       - 내가 생각하는 Ajax의 특징
         1. form으로 주고받지 않는듯. div로 해도되는듯하다.(맞음)
         2. data:의  타입은 contextType: (dataType: 아님)

     - 댓글 등록 성공 후 작성한 댓글 지워주기(textarea 비워두기)
   ...
   - 입력버튼 클릭시 ajax로 서버에 댓글 내용, 게시글 번호를 전송
     - /comment/insert

   - Controller에서 화면에서 전달한 내용을 VO에 담아서 확인
