...
//		}
	...
	/* 기능 : Scanner를 이용하여 수정할 학생의 정보와 성적, 이름을 입력하고
	 * 		 입력받은 학생 정보를 주어진 리스트에 수정하여 수정여부를 알려주는 메소드
	 * 매개변수 : Scanner, ArrayList<ExbStudent> stdList
	 * 리턴타입 : boolean (수정됐는지 안됐는지)
	 * 메소드명 : modifyStd*/
	public static boolean modifyStd(Scanner sc, ArrayList<ExbStudent> stdList) {
		System.out.println("학생정보를 입력하세요");
		System.out.println("학년");
		int grade = sc.nextInt();
		System.out.println("학반");
		...
		System.out.println("번호");
		int stdNum = sc.nextInt();
		ExbStudent tmpStd2 = new ExbStudent("", grade, stdClass, stdNum, 0, 0, 0);
		int index = stdList.indexOf(tmpStd2);
		if(index>=0) {
			sc.nextLine();
			System.out.println("이름");
			String name = sc.nextLine();
			...
			//방법1 : 리스트에서 학생 정보를 가져와서 이름과 성적만 수정
			/*tmpStd2 = students.get(index);
			tmpStd2.setStdName(name);
			tmpStd2.setKor(kor);
			tmpStd2.setEng(eng);
			tmpStd2.setMath(math);
			*/
			
			//방법2 : 입력받은 학생 정보와 이름, 성적을 하나의 객체로 만들어서 리스트에 수정하는 방법
			tmpStd2 = new ExbStudent(name, grade, stdClass, stdNum, kor, math, eng);
			stdList.set(index, tmpStd2);
			return true;
		}
		return false;
	}
